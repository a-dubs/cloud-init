        ...
        for root, _, files in os.walk(directory):
            for file in files:
                if file.endswith('.py'):
                    ...
                    file_info = {}
                    if analyze_type_hints:
                        file_info["functions_without_type_hints"] = functions_without_type_hints
                    if analyze_docstrings:
                        file_info["functions_without_docstrings"] = functions_without_docstrings
...
def analyze(directories, output_file, analyze_docstrings, analyze_type_hints):
    functions_info, total_number_of_functions = find_functions_info(directories, analyze_docstrings, analyze_type_hints)
...
def compare(existing_analysis_file, new_analysis_file):
    print(f"Comparing existing analysis ({existing_analysis_file}) with new analysis ({new_analysis_file})")
    ...
    for key in keys_to_compare:
        ...
        for file, data in new_info.items():
            ...
            existing_functions_set = set(existing_info.get(file, {}).get(key, []))
            added_funcs = new_functions_set - existing_functions_set
            if added_funcs:
                new_functions.extend([f"{file}: {func}" for func in added_funcs])
        if new_functions:
            errors.append(f'Error: Number of {key.replace("_", " ")} increased')
...
if __name__ == "__main__":
    ...
    analyze_parser.add_argument('--output-file', type=str, required=True, help='Output YAML file')
    analyze_parser.add_argument('--dir', type=str, action='append', required=True, help='Directory to analyze (can be specified multiple times)')
    analyze_parser.add_argument('--docstrings', action='store_true', help='Analyze docstrings')
    analyze_parser.add_argument('--type-hints', action='store_true', help='Analyze type hints')

    compare_parser = subparsers.add_parser('compare', help='Compare the results of two analyses.')
    compare_parser.add_argument('--existing-analysis', type=str, required=True, help='Existing/current analysis YAML file')
    compare_parser.add_argument('--new-analysis', type=str, required=True, help='New/modified analysis YAML file')
