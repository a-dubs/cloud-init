...
def main():
    parser = argparse.ArgumentParser(description="Mount a device using various identifiers")
    parser.add_argument('-l', '--label', help="Label of the partition to mount")
    parser.add_argument('-u', '--uuid', help="UUID of the partition to mount")
    parser.add_argument('-p', '--partuuid', help="PARTUUID of the partition to mount")
    parser.add_argument('-b', '--blockdevice', help="Block device path (e.g., /dev/sdb1)")
    parser.add_argument('-m', '--mountpoint', required=True, help="Mount point directory")
    ...
    # Get the device path
    device = get_device_path(label=args.label, uuid=args.uuid, partuuid=args.partuuid, block_device=args.blockdevice)
    if device:
        # Mount the device
...
def test_get_device_path():
    ...
    parser.add_argument('-l', '--label', help="Label of the partition to retrieve")
    parser.add_argument('-u', '--uuid', help="UUID of the partition to retrieve")
    parser.add_argument('-p', '--partuuid', help="PARTUUID of the partition to retrieve")
    parser.add_argument('-b', '--blockdevice', help="Block device path to retrieve")
